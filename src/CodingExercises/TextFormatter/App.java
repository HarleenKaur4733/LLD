 ...
 * easy
 * Build a text formatting system where different strategies format text in
 * different ways. The TextEditor context should
 * allow swapping formatters at runtime, so the same editor can produce
 * uppercase, lowercase, or title case output
 * depending on the active strategy.
 * 
 * Requirements:
 * 
 * Strategy interface: TextFormatter with a method format(text) that returns a
 * formatted string
 * Concrete strategies: UpperCaseFormatter, LowerCaseFormatter,
 * TitleCaseFormatter
 * Context: TextEditor with setFormatter() to swap strategies and publishText()
 * to format and print text
 */
